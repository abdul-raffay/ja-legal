...
$(document).ready(function () {
  ...
  if ($('#scroll-page').length) {
    $("#scroll-page").onepage_scroll({
      ...
      beforeMove: function beforeMove() {
        ...
        $('main section.home-banner:first-child .banner-overlay').removeClass('left-animate').addClass('bottom-animate'); //if($(window).width() < 767) {
  ...
  var headHeight = $('header').outerHeight(); // $('.menu-toggle').on('click', function() {
  ...
  if ($(".vertical-scroller").length) {
    ...
    $(".vertical-scroller").mCustomScrollbar({
      ...
      scrollEasing: "easeOut" //     callbacks:{
  ...
  if ($('#from').length) {
    ...
    $("#from").datepicker({
      ...
      onSelect: function onSelect(selected) {
        ...
        var myyear = $(this).datepicker('getDate').getFullYear() % 100; //console.log(mydate + " " +monthNamesShort[mymonth - 1] + " '" + myyear);
  ...
  $('.tab-wrp > ul li a').on('click', function () {
    ...
    setTimeout(function () {
      ...
      $('html, body').animate({
        scrollTop: scrollTo - headHeight - tabHeight
      }, 1000);
  ...
  $(window).resize(function () {
    ...
    var contentArea = $('.header-blue').outerWidth(); //console.log(contentArea + "total content area")

    setTimeout(function () {
      var blueArea = $('.banner-overlay').outerWidth(); //  console.log(blueArea + "blue area")

      var totalareaUse = contentArea - blueArea; //console.log(totalareaUse + "total area")
  ...
  if (isIE) {
    $('.hasDatepicker').on('click', function () {
      ...
      var _thisPos = $(this).offset().top - scrollPos; //var winHeight = $(window).height();
...
$(window).on('load', function () {
  ...
  if (urlPara[1] == "focus-sectors") {
    setTimeout(function () {
      $('html, body').animate({
        scrollTop: $('#focus-sectors').offset().top
      });
    }, 3000);
  }
...
if ($('.tab-wrp').length) {
  ...
  $(document).on('scroll', function () {
    ...
    var headHeight = $('header').outerHeight(); //console.log(headHeight)
...
$(window).on('scroll', function () {
  if (winW >= 768) {
    if ($('.tab-wrp').hasClass('active')) {
      ...
      $('.tab-wrp.active .download').css({
        'right': parseInt(downloadLeft) + 20
      });
    } else {
